...
int (*bind_socket_func)(void *tun_interface, int fd, const char *tag);
...
int bind_socket(void *interface, int fd, char *tag) {
    TRACE_METHOD();

    int result = bind_socket_func(interface, fd, tag);
